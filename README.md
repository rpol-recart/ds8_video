# Container Detection System (DeepStream 8 + PaddleOCR)
...
- NVIDIA Driver ≥ 535 (для DeepStream 8.0)
- RTSP-камера
...
2. **Redis TTL**: Распознанный ISO-номер сохраняется в Redis с TTL (по умолчанию 24 часа). Повторное появление того же номера в течение TTL игнорируется.
3. **Fuzzy matching**: Таблица типичных OCR-ошибок (O↔0, I↔1, S↔5, B↔8, Z↔2, G↔6) генерирует ограниченный набор вариантов и проверяет каждый через O(1) lookup в Redis, вместо полного сканирования.
